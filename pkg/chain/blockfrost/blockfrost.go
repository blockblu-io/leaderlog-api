...
// NewBlockFrostBackend is creating a new chain.Backend that uses
// Blockfrost API with the api key specified in the environment.
...
func (b *Backend) GetMintedBlock(ctx context.Context, slot uint) (*chain.MintedBlock, error) {
	...
	pool, err := b.cache.fetchPoolMetadata(ctx, block.SlotLeader, b.fetchPoolMetadata())
	if err != nil {
		return nil, err
	}
...
func (b *Backend) TraverseAround(ctx context.Context, slot uint, interval uint) (*chain.MintedBlock, error) {
